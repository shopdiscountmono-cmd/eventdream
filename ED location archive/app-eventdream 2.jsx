            ...
            {filtered.length === 0 ? (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  ...
                  <Btn variant="danger" size="sm" onClick={() => { if (confirm(`Supprimer ${client.name} ?`)) setClients(prev => prev.filter(c => c.id !== client.id)); }}>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        {isEd ? <>
                          ...
                          <Btn variant="danger" size="sm" onClick={() => { if (confirm("Supprimer ?")) setStock(prev => prev.filter(s => s.id !== item.id)); }}><span style={{ width: 13, height: 13 }}>{I.trash}</span></Btn>
                        </>}
                      </div>
                    </td>
                  </tr>
                ...
                {filtered.length === 0 ? <tr><td colSpan={8} style={{ textAlign: "center", padding: 40, color: "#999" }}>Aucune dépense</td></tr> : filtered.map((exp, idx) => {
                  ...
                  return (
                    <tr key={exp.id} style={{ borderTop: "1px solid #f0f0f0", background: idx % 2 === 0 ? "#fff" : "#fafafa" }}>
                      ...
                      <td style={{ padding: "12px 16px" }}><div style={{ display: "flex", gap: 6 }}><Btn variant="secondary" size="sm" onClick={() => openEdit(exp)}><span style={{ width: 13, height: 13 }}>{I.edit}</span></Btn><Btn variant="danger" size="sm" onClick={() => { if (confirm("Supprimer ?")) setExpenses(prev => prev.filter(e => e.id !== exp.id)); }}><span style={{ width: 13, height: 13 }}>{I.trash}</span></Btn></div></td>
                    </tr>
                  );
                })}
              </tbody>
...
export default function App() {
  ...
  const deleteOrder = (id) => { if (confirm("Supprimer cette commande ?")) setOrders(prev => prev.filter(o => o.id !== id)); };
